...
//    else if (tracksToShow === 'latestTrack'){
        // no from/to time stamp -> display only latest track (client)
        // -> ToDo ... move logic to server to save 
//    }
...
/*        query = {userId: userId,
                 $and: [ { timestamp: { $gte: fromTimestampQuery.getTime().toString() } },
    ...
    db.collection('geoDataUser').find(findRequest).toArray( function (err, result) {
        if (err) {
          ...
          return;
            ...
            if(displayName){
                responseText = result.length + " geo points of user '" + displayName + "' retrieved";
            }
            else{
                responseText = result.length + " geo points of user '" + userId + "' retrieved";
            }
