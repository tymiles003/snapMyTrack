                    ...
                    if(accessToken !== "undefined"){
                            ...
                            if (err) {
                                ...
                                if(result && result.length>0){     // expected to be one or none
                                    ...
                                    if(publishData.publishEndTimestamp > new Date()){
                                        getGeoDataOfUserByDate(db, res, publishData.publisherAccountType, publishData.publisherUserId, publishData.publisherDisplayName, publishData.publishStartTimestamp, null);
                                    }
                                    else{
                                        res.status(400).jsonp({
                        ...
                        db.collection('appUser').find(query).toArray( function (err, result) {
                            if (err) {
                                ...
                                getGeoDataOfUserByDate(db, res, accountType, userId, result[0].displayName, null, null);
                            }
                        });
                    }
                }
...
function getGeoDataOfUserByDate(db, res, accountType, userId, displayName,timestampFrom,timestampTo){
    // userGeoPoint
