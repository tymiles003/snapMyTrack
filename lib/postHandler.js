...
module.exports = {
  ...
  registerHandlerPublish: function(app){
    handlePublish(app);
    console.log('handlePublish registered');
  }
};
...
function handlePublish(app){
    app.post('/publish', function(req, res) {
        ...
        var publishData = {
          ...
          'to' : req.body.to,
          'from' : "michbier@gmail.com",     // req.body.from,
          'subject' : "SnapLoc: User "+req.body.userId+" has shared his location with you",    // req.body.subject,
          'html' : '<a href="http://geotracker-dsignmatters.rhcloud.com?userId='+req.body.userId+'">Geo Track of your friend '+req.body.userId+'</a>',   // req.body.html,
        };
        console.log('publishData: ');
        console.log(publishData);
        // validate geo point
        if ((publishData.userId === undefined) || (publishData.to === undefined)
            || (publishData.from === undefined) || (publishData.subject === undefined)
            || (publishData.html === undefined ) ) {
          res.status(400).jsonp({
            error : 'Bad request, parameter(s) undefined'
          });
          return;
        }
        mailService.sendSnapLocMail( publishData );
    });
}
