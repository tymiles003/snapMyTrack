...
function handleUserUpdatePublicDataPost(app){
    app.post('/userUpdate', function(req, res) {
        ...
        console.log(req.body);
        // validate password-reset data
        if ( req.body.accountType === undefined || req.body.userId === undefined || req.body.password === undefined || req.body.displayName === undefined ) {
          res.status(400).jsonp({
            error : "Bad request, parameter(s) undefined"
          });
          return;
        ...
        userValidateForUserUpdatePublicData(res, req.body.accountType, req.body.userId, req.body.password, req.body.displayName, pictureUrl, userValidateForUserUpdatePublicDataCallback);
    });
}
...
function userValidateForUserUpdatePublicData(res, accountType, userId, password, displayName, pictureUrl, callback){
    // created with assistance of http://blog.modulus.io/mongodb-tutorial
    var mongoClient= mongodb.MongoClient;
    var mongodb_db_url;
        ...
        if (err) {
            ...
            db.collection('appUser').find(query).toArray( function (err, result) {
                if (err) {
                    ...
                    if(result && result.length>0){     // expected to be one or none
                        callback(res, db, result[0], userId, displayName, pictureUrl);
                    }
                    else{
                        console.log('There is no user with Email ' + userId + ' and password ' +password );
                        callback(res, db, null, userId, displayName, pictureUrl);
                    }
                }
            });
        }
...
function userValidateForUserUpdatePublicDataCallback(res, db, appUser, userId, displayName, pictureUrl){
    // change password
    if(appUser){
