...
var dbUtilities = Promise.promisifyAll(require('../dbUtilities'));
...
function userValidateForAccountRemove(res, accountType, userId, password, callback){
    ...
    mongoClient.connect(mongodb_db_url, function (err, db) {
        if (err) {
            ...
            console.log('query (user validate for remove account): ', query);
            db.collection('appUser').find(query).toArray( function (err, result) {
                if (err) {
                    console.log('Reading application user from Mongo DB collection "appUser" has failed: '+err);
                    db.close();
...
function removeAccount(appUser, db, res){
    ...
    dbUtilities.removeUserFromFollowUserAsync(appUser, db, res)
//    .then( dbUtilities.removeUserFromPublishForUserAsync(appUser, db, res) )
//    .then( dbUtilities.removeUserFromGeoDataUserAsync(appUser, db, res) )
    .then( dbUtilities.removeUserFromUserSettingsAsync(appUser, db, res) )
//    .then( dbUtilities.removeUserFromAppUserAsync(appUser, db, res) )
    .then( dbUtilities.createSuccessMessage(appUser, db, res) )
